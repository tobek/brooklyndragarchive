...
<?php
		...
		// CHANGED BY TOBY $login->display_name to nickname
			$registerFilterArray['loginLogoutError'] = '
				<p class="log-in-out alert">'. __('You are logged in as', 'profilebuilder') .' <a href="'.get_author_posts_url( $login->ID ).'" title="'.$login->nickname.'">'.$login->nickname.'</a>. '. __('You don\'t need another account.', 'profilebuilder') .' <a href="'.wp_logout_url(get_permalink()).'" title="'. __('Log out of this account.', 'profilebuilder') .'">'. __('Logout', 'profilebuilder') .'  &raquo;</a></p><!-- .log-in-out .alert -->';
...
<?php 					
								...
								if ($wppb_defaultOptions['email'] == 'show'){
									...
									$registerFilterArray2['name2'] = '
										<p class="form-email'.$errorVar.'">
											<label for="email">'. __('Email', 'profilebuilder') .$errorMark.'</label>
						...
						if ($wppb_defaultOptions['nickname'] == 'show'){
							...
							// TOBY CHANGED Nickname to Username
							$registerFilterArray2['name5'] = '
								<p class="nickname'.$errorVar.'">
									<label for="nickname">'. __('Username', 'profilebuilder') .$errorMark.'</label>
						...
						if ($wppb_defaultOptions['password'] == 'show'){
							...
							if ($wppb_generalSettings['emailConfirmation'] != 'yes'){
								if (!is_multisite()){
									...
									$checkedVar = ' checked'; // ADDED BY TOBY this is hidden with CSS but let's always use it
