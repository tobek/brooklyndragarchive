...
function wppb_front_end_login( $atts ){
	...
	if ( is_user_logged_in() ){  // Already logged in 
		...
		// ADDED BY TOBY and commented out next block
		$display_name = $wppb_user->nickname ? $wppb_user->nickname : $wppb_user->user_email;
		/*
		if (isset($wppb_generalSettings['loginWith']) && ($wppb_generalSettings['loginWith'] == 'email'))
		...
		?>
			...
			<?php
				if ($display === true){
					...
					// CHANGED BY TOBY lost password message and adding register link
					$loginFilterArray['loginURL'] = '
						<p>
							Lost password? <a href="'.$siteURL.'">'. __('Create a new one', 'profilebuilder').'</a>
						</p>
						<p>
							Not registered? <a href="/register/">'. __('Sign up here', 'profilebuilder').'</a>
						</p>';
